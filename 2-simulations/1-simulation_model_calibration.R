...
for (k in 1:length(u_list)){
  ...
  set.seed(seed)
  u=u_list[k]
  simul=SimulateGraph(n=n, pk=pk, topology=topology, u=u, output_matrices=TRUE)
  contrasts=c(contrasts, GetContrast(simul$C))
...
{pdf(plotname, width=12, height=8)
  ...
  plot(log(u_list), contrasts, pch=19, col="navy", cex=0.5, 
       xlab="log(u)", ylab="Contrast", cex.lab=1.5, las=1)
  ...
  Heatmap(mycor[,,dim(mycor)[3]], colours=c("darkblue","white","firebrick3"), 
          legend_range=c(-1,1), legend=FALSE)
  Heatmap(mycor[,,which.max(contrasts)], colours=c("darkblue","white","firebrick3"), 
          legend_range=c(-1,1), legend=FALSE)
  Heatmap(mycor[,,1], colours=c("darkblue","white","firebrick3"), 
          legend_range=c(-1,1), legend_length=10, legend=TRUE)
  dev.off()}
