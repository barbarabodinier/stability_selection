...
set.seed(seed)
simul=SimulateGraph(n=n, pk=pk, topology=topology, nu=nu, v_within=v_w, v_between=v_b)
...
{pdf("Figures/2-simulations/Heatmaps_multi_block.pdf", width=12, height=11)
  ...
  for (i in 1:2){
    ...
    axis(side=i, at=mean(c(0.5,49.5)), labels=ifelse(i==1, yes="DNA methylation", no="Gene expression"), 
         tick=FALSE, cex.axis=1.5)
    axis(side=i, at=c(50.5,99.5), labels=NA)
    axis(side=i, at=mean(c(50.5,99.5)), labels=ifelse(i==1, yes="Gene expression", no="DNA methylation"), 
         tick=FALSE, cex.axis=1.5)
  }
  
  Heatmap(mypcor, colours=c("darkblue","white","darkred"), 
          legend_range=c(-1,1), legend_length=30)
  ...
  Heatmap(mycor, colours=c("darkblue","white","darkred"), 
          legend_range=c(-1,1), legend_length=50, legend=FALSE)
  mtext(side=2, at=nrow(mycor)*1.025, text="C", cex=3, las=1, line=0.5)
  for (i in 1:2){
    axis(side=i, at=c(0.5,49.5), labels=NA)
    axis(side=i, at=mean(c(0.5,49.5)), labels=ifelse(i==1, yes="Group 1", no="Group 2"), 
         tick=FALSE, cex.axis=1.5)
    axis(side=i, at=c(50.5,99.5), labels=NA)
    axis(side=i, at=mean(c(50.5,99.5)), labels=ifelse(i==1, yes="Group 2", no="Group 1"), 
         tick=FALSE, cex.axis=1.5)
  }
