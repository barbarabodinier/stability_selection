...
for (metric in c("precision","recall","F1_score")){
  {pdf(paste0("Figures/2-simulations/Boxplot_", metric, "_", simul_study_id, "_", topology, "_PFER_thr_", PFER_thr, ".pdf"), 
    ...
    for (simul_id in 1:3){
      ...
      axis(side=1, at=mean(c(1,7))+4, labels=eval(parse(text=paste0("expression(PFER[MB]<",PFER_thr,")"))), 
           las=1, line=3.5, tick=FALSE)
      axis(side=1, at=c(9,15)+4, labels=NA, las=2, line=3.5)
      axis(side=1, at=mean(c(9,15))+4, labels=eval(parse(text=paste0("expression(PFER[SS]<",PFER_thr,")"))), 
           las=1, line=3.5, tick=FALSE)
