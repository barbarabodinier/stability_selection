...
mycolours=c("chocolate", "sienna", "violetred3", "violetred4", pal_mb, pal_ss, "darkred", "firebrick", "red", "tomato")
mypch=c(rep(17,length(pi_list)),rep(17,length(pi_list)),18,18,18,18)
dimensionality=c("Low", "Intermediate", "High")

# Saving table
for (simul_id in 1:3){
  performances=readRDS(paste0("Results/2-simulations/2-variable_selection/Simulations_",simul_study_id,"_",topology,"/Performances_",simul_id,"_merged_PFER_thr_", PFER_thr,".rds"))
  
  tmpmedian=apply(performances, c(1,2), FUN=function(x){median(as.numeric(x))})
  mymedian=tmpmedian
  ...
  tmpiqr=apply(performances[,-1,], c(1,2), FUN=function(x){IQR(as.numeric(x))})
  myiqr=tmpiqr
...
mytable=cbind(rep(c("Bonferroni", "B-H", "Lambda_min", "lambda_1se", rep("",3), "MB", rep("",6), "SS", rep("",3), "Subsampling", "CPSS", "MB", "SS"),3), mytable)
...
for (metric in c("precision","recall","F1_score")){
  {pdf(paste0("Figures/2-simulations/Boxplot_", metric, "_", simul_study_id, "_", topology, "_PFER_thr_", PFER_thr, ".pdf"), 
    ...
    for (simul_id in 1:3){
      ...
      xseq=c(1:7, 9:15, 17:20)+5
      xseq=c(1:4,xseq)
      myylab=ifelse(simul_id==1, yes=metric, no="")
      if (myylab=="F1_score"){
        myylab=eval(parse(text="expression(F[1]*'-score')"))
      }
      boxplot(at=xseq, mylist, col=mycolours, boxcol="white", whiskcol=mycolours, staplecol=mycolours, 
      ...
      axis(side=1, at=xseq[(1:4)], las=2, 
           labels=c("Bonferroni", "B-H", 
                    eval(parse(text=paste0("expression(lambda[min])"))), 
                    eval(parse(text=paste0("expression(lambda[se])")))))
      axis(side=1, at=xseq[(1:(length(pi_list)*2))+4], labels=rep(pi_list, 2), las=2)
      axis(side=1, at=xseq[((length(xseq)-3):length(xseq))], labels=NA)
      axis(side=1, at=xseq[((length(xseq)-3):length(xseq))], las=2, line=0, tick=FALSE, #hadj=0.5,
           labels=c("Subsampling", "CPSS", 
                    eval(parse(text=paste0("expression(PFER[MB]<",PFER_thr,")"))), 
                    eval(parse(text=paste0("expression(PFER[SS]<",PFER_thr,")")))))
      axis(side=1, at=c(1,7)+5, labels=NA, las=2, line=3.5)
      axis(side=1, at=mean(c(1,7))+5, labels=eval(parse(text=paste0("expression(PFER[MB]<",PFER_thr,")"))), 
           las=1, line=3.5, tick=FALSE)
      axis(side=1, at=c(9,15)+5, labels=NA, las=2, line=3.5)
      axis(side=1, at=mean(c(9,15))+5, labels=eval(parse(text=paste0("expression(PFER[SS]<",PFER_thr,")"))), 
           las=1, line=3.5, tick=FALSE)
      abline(v=c(5,13,21),lty=3,col="black")
    }
    dev.off()}
}
