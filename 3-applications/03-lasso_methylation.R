...
tau_expl=0.7
n_predictors=50
incremental=Incremental(xdata=cpg, ydata=covars$case, 
                        stability=out, 
                        tau=tau_expl, K=K_expl, 
                        n_predictors = n_predictors)

# Explanatory performance of smoking (packyears)
expl_smoking=ExplanatoryPerformance(xdata = covars[, "packyrsTF", drop=FALSE], 
                                    ydata = covars$case, 
                                    family="binomial", 
                                    tau=tau_expl, K=K_expl)
...
# Figure parameters
mylwd <- 1.5
mycolours=c(rep("navy", sum(SelectedVariables(out))), 
...
plotname <- "Figures/3-applications/Methylation_lung_cancer_lasso.pdf"
{
  ...
  plot(NA, bty="n", xlim=c(1, ncol(cpg)), ylim=c(0,1),
  ...
  plot(selprop,
       type = "h", col = c(rep("navy", sum(SelectedVariables(out))), 
