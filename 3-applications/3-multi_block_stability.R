...
PFER_thr=150
system.time({out=GraphicalModel(data=omic, pk=pk, lambda_other_blocks=0.1, Lambda_cardinal=30, 
...
mygraph=Graph(adjacency=adjacency, node_colour=c(rep("skyblue",ncol(cpg)), rep("lightsalmon", ncol(ttx))), 
...
# Make igraph object
node_label=paste(omicannot$alt.name, "\n", omicannot$name)
mygraph=Graph(adjacency=adjacency, node_colour=colours[chr_number], 
              node_label=node_label, node_shape=c(rep("square",ncol(cpg)), rep("circle", ncol(ttx))))
V(mygraph)$size=0.7*V(mygraph)$size
...
{pdf(paste0("Figures/3-applications/Multi_omics_graph_chromosomes_PFER_",PFER_thr,".pdf"), width=14, height=myasp*14)
  ...
  tmp[tmp=="23"]="X"
  legend("topleft", pch=19, col=colors[sort(unique(chr_number))], 
...
annot=annot[,-1]
colnames(annot)=c("MappedID", "Protein_name", "Panther_family", "Protein_class",
...
for (i in 1){
  ...
  number_of_hits=sapply(mylist_to_explore,
                        FUN=function(x){sum(unique(panther$MappedID[grep(x, eval(parse(text=paste0("panther$", genegroup[i]))), fixed=TRUE)])%in%omicannot[V(g)$name,1])})
  ...
  for (k in 1:length(mylist_to_investigate)){
    ...
    {pdf(paste0(filepath_group, "Multi_omics_graph_PFER_",sum(PFER_thr),"_",genegroup[i],"_",
                gsub("/","_",gsub(" ", "_", mylist_to_investigate[k])), ".pdf"), width=14, height=myasp*14)
      par(mar=rep(0,4))
      set.seed(myseed)
...
mygraph=Graph(adjacency=adjacency, node_colour=mynode_colours, 
              node_label=node_label, node_shape=c(rep("square",ncol(cpg)), rep("circle", ncol(ttx))))
V(mygraph)$size=0.7*V(mygraph)$size
...
{pdf(paste0("Figures/3-applications/Multi_omics_graph_PFER_",PFER_thr,".pdf"), width=14, height=myasp*14)
  ...
  set.seed(myseed)
  plot(mygraph, layout=layout_with_fr(mygraph), asp=myasp)
  legend("bottomleft", pch=c(15, rep(19,5)), pt.cex=2, cex=1.5, 
