...
# Simulation
set.seed(seed)
simul=SimulateGraph(n=n, pk=pk, topology=topology, nu=nu)
...
foo_mb=function(){
  set.seed(1)
  stab_mb <- stabsel(x=simul$data, fitfun=glasso.graphical_model, sampling.type="MB", mc.cores=1,
...
foo_mb=function(){
  set.seed(1)
  stab_mb <- stabsel(x=simul$data, fitfun=glasso.graphical_model, sampling.type="SS", assumption="unimodal", mc.cores=1,
                     cutoff=pi_list[k], PFER=PFER_thr, args.fitfun=list(lams=Lambda_single))
  assign("stab_mb", stab_mb, envir=.GlobalEnv)
}
perf_mb=NULL
...
foo_unconstr=function(){
out=GraphicalModel(data=simul$data, Lambda=Lambda_single, start="cold", PFER_method="SS")
assign("out", out, envir=.GlobalEnv)
}
tmptime=system.time(foo_unconstr())
A=Adjacency(out)
...
foo_unconstr=function(){
out=GraphicalModel(data=simul$data, Lambda=Lambda_single, start="cold", PFER_method="MB", PFER_thr=PFER_thr)
assign("out", out, envir=.GlobalEnv)
}
tmptime=system.time(foo_unconstr())
A=Adjacency(out)
...
foo_unconstr=function(){
out=GraphicalModel(data=simul$data, Lambda=Lambda_single, start="cold", PFER_method="SS", PFER_thr=PFER_thr)
assign("out", out, envir=.GlobalEnv)
}
tmptime=system.time(foo_unconstr())
A=Adjacency(out)
