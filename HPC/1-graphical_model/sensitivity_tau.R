...
# Lambda path
Lambda_single=LambdaGridNetwork(data=simul$data, lambda_path_refined_cardinal=100)

# Subsampling, different tau values
foo_unconstr=function(){
out=GraphicalModel(data=simul$data, Lambda=Lambda_single, start="cold", tau=tau, PFER_method="MB")
...
foo_unconstr=function(){
out=GraphicalModel(data=simul$data, Lambda=Lambda_single, start="cold", tau=0.5, PFER_method="SS")
assign("out", out, envir=.GlobalEnv)
}
tmptime=system.time(foo_unconstr())
A=Adjacency(out)
...
# Bootstrap
foo_unconstr=function(){
out=GraphicalModel(data=simul$data, Lambda=Lambda_single, start="cold", resampling="bootstrap")
assign("out", out, envir=.GlobalEnv)
}
tmptime=system.time(foo_unconstr())
A=Adjacency(out)
