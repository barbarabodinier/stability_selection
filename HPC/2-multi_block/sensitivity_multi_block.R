...
filepath <- paste0("../../Results/2-multi_block/Sensitivity_", simul_study_id, "_", topology, "/")
...
# Multi-parameters
foo_unconstr <- function() {
  out <- GraphicalModel(xdata = simul$data, pk = pk, Lambda = Lambda_blocks, lambda_other_blocks = NULL, start = "cold", PFER_method = "MB")
  assign("out", out, envir = .GlobalEnv)
}
tmptime <- system.time(foo_unconstr())
print(tmptime)
