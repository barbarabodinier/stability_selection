...
do
echo $j
sed "s/{simul_study_id_input}/${simul_study_id}/g" template_multi_block_calibration.sh > run1.sh
...
qsub run.sh
sed "s/{simul_study_id_input}/${simul_study_id}/g" template_single_block_calibration.sh > run1.sh
sed "s/{topology_input}/${topology}/g" run1.sh > run2.sh
sed "s/{do_exp_input}/${do_exp}/g" run2.sh > run3.sh
sed "s/{params_id_input}/${j}/g" run3.sh > run4.sh
sed "s/{PFER_thr_input}/${PFER_thr}/g" run4.sh > run.sh
